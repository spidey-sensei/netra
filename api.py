...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    version = _s3_get_stable(f"bank/{bank_name}")
...
async def upload_aadhar(file: UploadFile = File(...)):
    ...
    # 🔥 Aadhaar prompt from local file (NO S3 JSON needed)
    prompt = _load_local_aadhar_prompt()
    version = "local-1.0"
    return {
        "file_id": file_id,
        "document_type": "aadhar",
        "active_version": version,
        "active_prompt": prompt,   # 🔥 THIS IS THE KEY FIX
...
def _load_local_aadhar_prompt():
    try:
        with open("internal_prompt.txt", "r", encoding="utf-8") as f:
            return f.read()
    except Exception:
        return ""
...
@app.get("/active-prompt/{bank_name}")
def get_active_prompt(bank_name: str):
